...
from training.policy_runtime import choose_nn_action, load_nn_policy


def main() -> int:
    parser = argparse.ArgumentParser(description="Run a tiny neural BC policy against battle-test mode.")
    ...
    parser.add_argument("--policy", default=str(Path("Tools/python/checkpoints/battle_nn_bc_policy.json")))
    parser.add_argument("--steps", type=int, default=200)
    args = parser.parse_args()

    network = load_nn_policy(args.policy)
    ...
    for step in range(args.steps):
        if snapshot.get("sceneType") == "map" and (snapshot.get("run") or {}).get("battlesWon", 0) > 0:
            print("nn-bc stop: battle-test run finished and returned to map")
            return 0

        action = choose_nn_action(network, snapshot)
        if action is None:
        ...
        print(f"nn-bc step {step}: {action}")
