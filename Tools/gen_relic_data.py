...
# Generate the C# dictionary entries
entries = []
for r in relics:
    rid = r['id']
    nk = f'relic.{rid}.name'
    dk = f'relic.{rid}.description'
    name_zh = zh.get(nk, r['name'])
    desc_zh = zh.get(dk, r['description'])
    # Escape for C# string
    name_zh = name_zh.replace('\\', '\\\\').replace('"', '\\"')
    desc_zh = desc_zh.replace('\\', '\\\\').replace('"', '\\"')
    entries.append(f'        {{"{rid}", ("{name_zh}", "{desc_zh}")}},')
...
# Write the file
path = os.path.join('Scripts', 'Data', 'RelicData.cs')
with open(path, 'w', encoding='utf-8', newline='') as f:
    f.write(csharp_code)

print(f'Written {path} with {len(entries)} relic translations')
